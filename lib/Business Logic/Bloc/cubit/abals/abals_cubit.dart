...
class AbalsListCubit extends Cubit<AbalListState> {
  ...
  Future<List> getAbals() async {
    print('a======================abal called');
    emit(state.copyWith(
        abalListStatus: AbalListStatus.initial, errorMessage: ''));

    List<dynamic> abals = [];
    try {
      emit(state.copyWith(
          abalListStatus: AbalListStatus.loading, errorMessage: ''));
      ...
      emit(state.copyWith(
          abalListStatus: AbalListStatus.error, errorMessage: e.toString()));
